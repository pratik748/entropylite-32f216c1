/**
 * Foresight surface — the docked command console through which Foresight
 * operates the terminal. Ledger-style, monochrome, hairline construction:
 * an activity register, not a chat bubble UI. The interface itself (tabs,
 * modules, highlights) is where results are shown; this panel carries the
 * conversation, the execution ledger, evidence, and confirmations.
 */

import { useCallback, useEffect, useRef, useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { ArrowUpRight, ChevronRight, Command, Square, X } from "lucide-react";
...
export default function ForesightSurface() {
  ...
  // Consume prefill from the command palette.
  useEffect(() => {
    if (prefill) {
      setDraft(prefill);
      setPrefill("");
  ...
  return (
    <AnimatePresence initial={false}>
      {open && (
        ...
        >
          <div className="flex h-full w-[400px] flex-col min-h-0">
            {/* Header */}
            ...
            <div className="shrink-0 border-t border-border/60 px-3 py-2.5">
              <div className="flex items-center gap-2 rounded-md border border-border/70 bg-surface-2/50 px-2.5 h-9 focus-within:border-border transition-colors">
                ...
                <input
                  ...
                  placeholder={busy ? "Working…" : "Ask, compare, simulate, operate…"}
                  disabled={busy}
