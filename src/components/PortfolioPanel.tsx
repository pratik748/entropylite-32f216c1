...
const PortfolioPanel = ({
  ...
  const totalCurrent = stocks.reduce(
    (sum, s) =>
      sum + (s.analysis?.currentPrice ?? s.buyPrice) * s.quantity,
    0
  );
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 animate-slide-up">
      ...
      {analyzedCount > 0 && (
        <div className="mb-4 rounded-lg bg-surface-2 p-3">
          ...
          <div className="flex items-center justify-between mt-1">
            ...
            >
              {totalPnL >= 0 ? "+" : ""}₹{totalPnL.toLocaleString("en-IN", { maximumFractionDigits: 0 })} ({totalPnLPct >= 0 ? "+" : ""}{totalPnLPct.toFixed(1)}%)
            </span>
          </div>
        </div>
      )}
      ...
      <div className="space-y-2 max-h-[400px] overflow-y-auto">
        {stocks.map((stock) => {
          const pnl = stock.analysis
            ? (stock.analysis.currentPrice - stock.buyPrice) * stock.quantity
            : 0;
          const pnlPct = stock.analysis
            ? ((stock.analysis.currentPrice - stock.buyPrice) / stock.buyPrice) * 100
            : 0;
            ...
            <div
              ...
              className={`group flex items-center justify-between rounded-lg border p-3 cursor-pointer transition-all ${
                isActive
                  ...
                  : "border-border/50 bg-surface-2 hover:border-border hover:bg-surface-3"
              }`}
            >
                  ...
                  {stock.analysis && (
                    <span
                      className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${
                        stock.analysis.suggestion === "Add"
                          ? "bg-gain/10 text-gain"
                          : stock.analysis.suggestion === "Exit"
                          ? "bg-loss/10 text-loss"
                          : "bg-warning/10 text-warning"
                      }`}
                    >
                      {stock.analysis.suggestion}
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-2 mt-0.5 text-xs text-muted-foreground">
                  <span>{stock.quantity} qty</span>
                  <span>@ ₹{stock.buyPrice.toLocaleString("en-IN")}</span>
                </div>
              </div>
                ...
                {stock.analysis && (
                  <div className="text-right">
                    ...
                    <span className="font-mono text-[10px] text-muted-foreground">
                      ₹{stock.analysis.currentPrice.toLocaleString("en-IN")}
                    </span>
                  </div>
                )}
                <button
      ...
      {stocks.length === 0 && (
        <div className="py-8 text-center">
          ...
          <p className="text-xs text-muted-foreground/60 mt-1">
            Add a stock using the form above
          </p>
        </div>
      )}
    </div>
