...
import { WORKSPACES, WORKSPACE_GROUPS, sectionPath } from "./registry";
...
/**
 * Workstation navigation rail — 9 workspaces in 4 semantic groups.
  ...
  return (
    <>
      ...
      >
        {WORKSPACE_GROUPS.map((group) => (
          <div key={group} className="mb-1.5">
            ...
            {WORKSPACES.filter((w) => w.group === group).map((workspace) => {
      ...
      <div className="md:hidden">
        ...
        <SectionStrip ticker={ticker} activeWorkspaceId={activeWorkspaceId} activeSectionId={activeSectionId} />
      </div>
    </>
  );
};

const SectionStrip = ({ ticker, activeWorkspaceId, activeSectionId }: SectionRailProps) => {
