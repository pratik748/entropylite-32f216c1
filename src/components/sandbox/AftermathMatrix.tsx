...
const AftermathMatrix = ({ stocks }: Props) => {
  ...
  const results = useMemo(() => {
    ...
    const stockImpacts = holdings.map(h => {
      ...
      const marketCap = 100000000000;
      const positionPctOfMCap = (h.value / marketCap) * 100;
      const priceImpactBps = Math.sqrt(positionPctOfMCap) * 15;
      const priceImpactPct = priceImpactBps / 100;
      const estimatedADV = marketCap * 0.003;
      const daysToUnwind = h.value / (estimatedADV * 0.1);
    ...
    <div className="space-y-5">
      <div className="grid gap-3 grid-cols-2 md:grid-cols-4">
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <p className="mt-1 font-mono text-xl font-bold text-loss">{fmt(results.totalSlippage)}</p>
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        <h3 className="text-sm font-bold text-foreground uppercase tracking-wider mb-4"><Crosshair className="inline h-4 w-4 mr-2" />Price Impact by Position (bps)</h3>
        <div className="h-56">
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border">
                {["Asset", "Weight", "Impact (bps)", `Slippage ${sym}`, "Unwind Days", "Narrative", "Competitor", "Optimal Size"].map(h => (
            ...
            <tbody>
              {results.stockImpacts.map(s => (
                <tr key={s.ticker} className="border-b border-border/50 hover:bg-surface-2 transition-colors">
                  ...
                  <td className="px-3 py-2 font-mono text-loss">{fmt(s.slippageCost)}</td>
                  <td className="px-3 py-2 font-mono text-foreground">{s.daysToUnwind}</td>
                  <td className="px-3 py-2">
                    <span className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${s.narrativeRisk === "High" ? "bg-loss/10 text-loss" : s.narrativeRisk === "Medium" ? "bg-warning/10 text-warning" : "bg-gain/10 text-gain"}`}>{s.narrativeRisk}</span>
                  </td>
