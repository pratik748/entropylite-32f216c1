    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-3 grid-cols-2 md:grid-cols-4">
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <p className="mt-1 font-mono text-lg font-bold text-loss">{fmt(results.totalSlippage)}</p>
          ...
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-card">
              <tr className="border-b border-border">
                {["Time", `Slice ${sym}`, `Cumulative ${sym}`, "% Filled", "Impact (bps)", `Cum. Slippage ${sym}`].map(h => (
            ...
            <tbody>
              {results.executionPath.map((e, i) => (
                <tr key={i} className="border-b border-border/50">
                  ...
                  <td className="px-3 py-1.5 font-mono text-muted-foreground">{fmt(e.sliceSize)}</td>
                  <td className="px-3 py-1.5 font-mono text-foreground">{fmt(e.filled)}</td>
                  ...
                  <td className="px-3 py-1.5 font-mono text-loss">{fmt(e.slippage)}</td>
                </tr>
              ))}
            </tbody>
          </table>
