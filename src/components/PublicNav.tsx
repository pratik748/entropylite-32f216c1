    ...
    <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-black/5">
      <div className="max-w-6xl mx-auto flex items-center justify-between px-4 sm:px-6 h-14">
        ...
        <div className="hidden sm:flex items-center gap-6">
          {NAV_LINKS.filter((l) => l.path !== location.pathname).map((l) => (
            <button
              ...
              className="font-mono text-[11px] text-black/50 hover:text-black transition-colors"
            >
              {l.label}
            </button>
          ))}
        ...
        <div className="flex sm:hidden items-center gap-2">
          <Button
            size="sm"
            className="bg-black text-white hover:bg-black/85 font-mono text-[10px] h-9 px-3"
            onClick={() => navigate("/dashboard")}
          >
            Sign In
          </Button>
          <button
            onClick={() => setOpen((p) => !p)}
            className="p-2 -mr-2 text-black/60"
            aria-label="Menu"
          >
      ...
      {/* Mobile drawer */}
      {open && (
        <div className="sm:hidden border-t border-black/5 bg-white px-4 pb-4 pt-2 space-y-1 animate-in slide-in-from-top-2 duration-200">
          {NAV_LINKS.map((l) => (
            <button
              key={l.path}
              onClick={() => { navigate(l.path); setOpen(false); }}
              className={`block w-full text-left font-mono text-sm py-3 px-3 rounded-lg transition-colors ${
          ...
          <div className="pt-2 border-t border-black/5 mt-2">
            <p className="font-mono text-[9px] text-black/30 tracking-wide px-3">by Pratik Sehwag</p>
          </div>
        </div>
      )}
    </nav>
