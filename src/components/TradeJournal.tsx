...
const TradeJournal = () => {
  ...
  const [form, setForm] = useState({
    ...
    date: new Date().toISOString().split("T")[0], notes: "", fees: "",
  });

  const handleAdd = () => {
    if (!form.ticker || !form.price || !form.quantity) return;
    setTrades((prev) => [{
      id: crypto.randomUUID(), ticker: form.ticker.toUpperCase(), type: form.type,
      price: parseFloat(form.price), quantity: parseInt(form.quantity),
      date: form.date, notes: form.notes, fees: form.fees ? parseFloat(form.fees) : undefined,
    }, ...prev]);
    setForm({ ticker: "", type: "BUY", price: "", quantity: "", date: new Date().toISOString().split("T")[0], notes: "", fees: "" });
    setShowForm(false);
  };

  const handleRemove = (id: string) => { setTrades((prev) => prev.filter((t) => t.id !== id)); };
  ...
  trades.forEach((t) => {
    ...
    if (t.type === "BUY") { pnlByTicker[t.ticker].bought += t.price * t.quantity; pnlByTicker[t.ticker].buyQty += t.quantity; }
    else { pnlByTicker[t.ticker].sold += t.price * t.quantity; pnlByTicker[t.ticker].sellQty += t.quantity; }
    ...
    <div className="space-y-6">
      {trades.length > 0 && (
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <SummaryCard label="Total Fees" value={fmt(totalFees)} />
        </div>
      )}
          ...
          <div className="flex rounded-lg border border-border overflow-hidden">
            {(["ALL", "BUY", "SELL"] as const).map((f) => (
              <button key={f} onClick={() => setFilter(f)}
                className={`px-3 py-1.5 text-xs font-medium transition-colors ${filter === f ? "bg-primary text-primary-foreground" : "bg-surface-2 text-muted-foreground hover:bg-surface-3"}`}>
                {f}
              </button>
            ))}
          </div>
          <Button onClick={() => setShowForm(!showForm)} size="sm" className="gap-1.5"><Plus className="h-3.5 w-3.5" /> Log Trade</Button>
        </div>
      </div>
      ...
      {showForm && (
        <div className="rounded-xl border border-border bg-card p-5 animate-slide-up">
          <div className="grid gap-3 md:grid-cols-3">
            <Input placeholder="Ticker" value={form.ticker} onChange={(e) => setForm({ ...form, ticker: e.target.value })} className="bg-surface-2 border-border font-mono text-sm" />
            <div className="flex rounded-lg border border-border overflow-hidden">
              <button onClick={() => setForm({ ...form, type: "BUY" })} className={`flex-1 px-3 py-2 text-sm font-medium transition-colors ${form.type === "BUY" ? "bg-gain/20 text-gain" : "bg-surface-2 text-muted-foreground"}`}>BUY</button>
              <button onClick={() => setForm({ ...form, type: "SELL" })} className={`flex-1 px-3 py-2 text-sm font-medium transition-colors ${form.type === "SELL" ? "bg-loss/20 text-loss" : "bg-surface-2 text-muted-foreground"}`}>SELL</button>
            </div>
            <Input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className="bg-surface-2 border-border text-sm" />
          </div>
          <div className="grid gap-3 md:grid-cols-3 mt-3">
            <Input placeholder={`Price ${sym}`} type="number" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} className="bg-surface-2 border-border font-mono text-sm" />
            <Input placeholder="Quantity" type="number" value={form.quantity} onChange={(e) => setForm({ ...form, quantity: e.target.value })} className="bg-surface-2 border-border font-mono text-sm" />
            <Input placeholder={`Fees ${sym} (optional)`} type="number" value={form.fees} onChange={(e) => setForm({ ...form, fees: e.target.value })} className="bg-surface-2 border-border font-mono text-sm" />
          </div>
          <Input placeholder="Notes (optional)" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="bg-surface-2 border-border text-sm mt-3" />
          <Button onClick={handleAdd} className="mt-3 w-full" disabled={!form.ticker || !form.price || !form.quantity}>Log Trade</Button>
        </div>
      )}
              ...
              {filtered.map((trade) => (
                <tr key={trade.id} className="border-b border-border/50 hover:bg-surface-2 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center gap-1 rounded px-2 py-0.5 text-xs font-medium ${trade.type === "BUY" ? "bg-gain/10 text-gain" : "bg-loss/10 text-loss"}`}>
                      {trade.type === "BUY" ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
                      {trade.type}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right font-mono text-sm">{fmt(trade.price)}</td>
                  <td className="px-4 py-3 text-right font-mono text-sm">{trade.quantity}</td>
                  <td className="px-4 py-3 text-right font-mono text-sm font-semibold">{fmt(trade.price * trade.quantity)}</td>
